...
typedef TFunction<TUniquePtr<FHttpServerResponse>(
...
class FBaseHandler {
public:
  static TUniquePtr<FHttpServerResponse>
  AuthToken(const FHttpServerRequest &Request);
};

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FTokenReceived, const FString &,
                                            token);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FUserInfoReceived, const FString &,
                                            userId);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FValidateTokenReceived, bool,
                                            success);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGetUserIdFailed, const FString &,
                                            message);
...
public:
  ...
  FString token;
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Setup")
  TArray<FString> scopes = {"chat:read",
                            "chat:edit",
                            "bits:read",
  ...
  UPROPERTY(BlueprintAssignable, Category = "Message Events")
  FGetUserIdFailed OnGetUserIdFailed;

  // Global static storage because only on http listener can exist on a port so
  // it must apply to all existing components
...
private:
  ...
  void ProcessGetUserInfo(FHttpRequestPtr Request, FHttpResponsePtr Response,
                          bool bWasSuccessful);
  void ProcessTokenValidateInfo(FHttpRequestPtr Request,
...
public:
   ...
   */
  static FHttpRouteHandle BindRoute(const TSharedPtr<IHttpRouter> &HttpRouter,
                                    FString Path,
   ...
   * CreateHandler method is used to wrap the responser, in order to avoid the
   * crash
   */
  ...
  GetRequestJsonBody(const FHttpServerRequest &Request);
   ...
   */
  static TUniquePtr<FHttpServerResponse>
  SuccessResponse(TSharedPtr<FJsonObject> Data, FString Message);

  /**
   * Success response (data only)
   */
  static TUniquePtr<FHttpServerResponse>
  SuccessResponse(TSharedPtr<FJsonObject> Data);
   ...
   */
  static TUniquePtr<FHttpServerResponse>
  ErrorResponse(TSharedPtr<FJsonObject> Data, FString Message, int32 Code);

  /**
   * Error response (data & message)
   */
  static TUniquePtr<FHttpServerResponse>
  ErrorResponse(TSharedPtr<FJsonObject> Data, FString Message);
...
private:
  ...
  /**
   * Create json response from data, message, success status and user defined
   * error code
   */
  static TUniquePtr<FHttpServerResponse>
  JsonResponse(TSharedPtr<FJsonObject> Data, FString Message, bool Success,
               int32 Code);
