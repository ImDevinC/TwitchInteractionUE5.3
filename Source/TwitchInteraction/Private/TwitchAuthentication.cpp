...
void UTwitchAuthentication::BeginPlay() { Super::BeginPlay(); }
...
// Called every frame
void UTwitchAuthentication::TickComponent(
...
bool UTwitchAuthentication::FetchUserInfo(FString _username,
                                          FString _twitchToken) {
  FHttpModule *Http = &FHttpModule::Get();

  FHttpRequestRef Request = Http->CreateRequest();
  Request->OnProcessRequestComplete().BindUObject(
      this, &UTwitchAuthentication::ProcessGetUserInfo);
...
void UTwitchAuthentication::Init() {
  ...
  TSharedPtr<IHttpRouter> HttpRouter =
      HttpServerModule->GetHttpRouter(listenPort);
  BindRouters(HttpRouter);
  HttpServerModule->StartAllListeners();
  FString Url = FString("https://id.twitch.tv/oauth2/authorize?client_id=") +
                clientId + FString("&redirect_uri=") + redirectUrl +
                FString("&response_type=token&scope=") +
                FString::Join(scopes, TEXT(" "));
  FPlatformProcess::LaunchURL(*Url, NULL, NULL);
}

void UTwitchAuthentication::BindRouters(
    const TSharedPtr<IHttpRouter> &HttpRouter) {
  FWebUtil::BindRoute(HttpRouter, TEXT("/Auth"),
                      EHttpServerRequestVerbs::VERB_GET,
                      FBaseHandler::AuthToken);
}

void UTwitchAuthentication::ProcessGetUserInfo(FHttpRequestPtr Request,
                                               FHttpResponsePtr Response,
                                               bool bWasSuccessful) {
  if (bWasSuccessful) {
    FString body = Response->GetContentAsString();
    if (!EHttpResponseCodes::IsOk(Response->GetResponseCode())) {
      OnGetUserIdFailed.Broadcast(*body);
      return;
    }
    FTwitchAuthUserInfo userInfo;
    FJsonObjectConverter::JsonObjectStringToUStruct(body, &userInfo, 0, 0);
    UE_LOG(HttpAuthLog, Warning, TEXT("Processing user info %s"), *body);
...
void UTwitchAuthentication::ProcessTokenValidateInfo(FHttpRequestPtr Request,
                                                     FHttpResponsePtr Response,
                                                     bool bWasSuccessful) {
  if (bWasSuccessful) {
    FString body = Response->GetContentAsString();
...
TUniquePtr<FHttpServerResponse>
FBaseHandler::AuthToken(const FHttpServerRequest &Request) {
  if (!Request.QueryParams.IsEmpty()) {
    ...
    if (UTwitchAuthentication::GlobalPubSubComponents.Num() > 0) {
      for (int i = 0; i < UTwitchAuthentication::GlobalPubSubComponents.Num();
           ++i) {
    ...
    if (UTwitchAuthentication::GlobalEventSubComponents.Num() > 0) {
      for (int i = 0; i < UTwitchAuthentication::GlobalEventSubComponents.Num();
           ++i) {
    ...
    if (UTwitchAuthentication::GlobalTwitchChatComponents.Num() > 0) {
      for (int i = 0;
           i < UTwitchAuthentication::GlobalTwitchChatComponents.Num(); ++i) {
        UTwitchAuthentication::GlobalTwitchChatComponents[i]->OAuthToken =
            token;
      }
    }
...
FHttpRouteHandle FWebUtil::BindRoute(const TSharedPtr<IHttpRouter> &HttpRouter,
                                     FString Path,
                                     const EHttpServerRequestVerbs &Verb,
                                     const FHttpResponser &HttpResponser) {
  ...
  if (!HttpPath.IsValidPath()) {
    ...
    if (GEngine != nullptr) {
      GEngine->AddOnScreenDebugMessage(
          -1, 10.0f, FColor::Red,
          FString::Printf(TEXT("Bind HTTP router failed! invalid path: %s"),
                          *Path));
    }
#endif
    return nullptr;
  ...
  // bind router
  auto RouteHandle = HttpRouter->BindRoute(
      HttpPath, Verb, FWebUtil::CreateHandler(HttpResponser));
  if (RouteHandle == nullptr) {
    UE_LOG(HttpAuthLog, Warning, TEXT("Bind failed: %s\t%s"), *VerbString,
           *Path);
    return nullptr;
  }
#if WITH_EDITOR
...
FHttpRequestHandler
FWebUtil::CreateHandler(const FHttpResponser &HttpResponser) {
  return [HttpResponser](const FHttpServerRequest &Request,
...
FWebUtil::GetRequestJsonBody(const FHttpServerRequest &Request) {
  // check if content type is application/json
  bool IsUTF8JsonContent = IsUTF8JsonRequestContent(Request);
  if (!IsUTF8JsonContent) {
    UE_LOG(HttpAuthLog, Warning,
           TEXT("caught request not in utf-8 application/json body content!"));
    return nullptr;
  }
  ...
  FString RequestBodyString =
      FString(UTF8_TO_TCHAR(RequestBodyBytes.GetData()));
#if WITH_EDITOR
  if (GEngine != nullptr) {
  ...
  TSharedRef<TJsonReader<>> JsonReader =
      TJsonReaderFactory<>::Create(RequestBodyString);
  TSharedPtr<FJsonObject> RequestBody;
  if (!FJsonSerializer::Deserialize(JsonReader, RequestBody)) {
    UE_LOG(HttpAuthLog, Warning,
           TEXT("failed to parse request string to json: %s"),
           *RequestBodyString);
    return nullptr;
  }
  return RequestBody;
...
UStructType *
FWebUtil::GetRequestUStructBody(const FHttpServerRequest &Request) {
...
TUniquePtr<FHttpServerResponse>
FWebUtil::SuccessResponse(TSharedPtr<FJsonObject> Data, FString Message) {
  return JsonResponse(Data, Message, true, SUCCESS_CODE);
}

TUniquePtr<FHttpServerResponse>
FWebUtil::SuccessResponse(TSharedPtr<FJsonObject> Data) {
...
TUniquePtr<FHttpServerResponse>
FWebUtil::ErrorResponse(TSharedPtr<FJsonObject> Data, FString Message,
                        int32 Code) {
...
TUniquePtr<FHttpServerResponse> FWebUtil::ErrorResponse(FString Message) {
  return ErrorResponse(MakeShareable(new FJsonObject()), Message,
                       DEFAULT_ERROR_CODE);
}
...
FString
FWebUtil::GetHttpVerbStringFromEnum(const EHttpServerRequestVerbs &Verb) {
  switch (Verb) {
...
TUniquePtr<FHttpServerResponse>
FWebUtil::JsonResponse(TSharedPtr<FJsonObject> Data, FString Message,
                       bool Success, int32 Code) {
...
TUniquePtr<FHttpServerResponse> FWebUtil::JsResponse(bool Success, int32 Code) {
  FString PageResponse =
      "<!DOCTYPE html PUBLIC\"ISO / IEC 15445:2000//DTD "
      "HTML//EN\"><html><head><title>UE App</title></head><body><p>Attempting "
      "to grab "
      "token...</p></"
      "body><script>window.onload=function(){window.location.href=window."
      "location.href.replace('#','?');}</script></html>";
...
TUniquePtr<FHttpServerResponse> FWebUtil::OkResponse(bool Success, int32 Code) {
  FString PageResponse =
      "<!DOCTYPE html PUBLIC\"ISO / IEC 15445:2000//DTD "
      "HTML//EN\"><html><head><title>UE App</title></head><body><p>Token "
      "received...</p></"
      "body><script>window.onload=function(){window.close();}</script></html>";
  return FHttpServerResponse::Create(PageResponse, TEXT("text/html"));
}
  ...
  for (auto &HeaderElem : Request.Headers) {
    ...
    if (LowerKey == TEXT("content-type")) {
      for (auto &Value : HeaderElem.Value) {
        ...
        // not strict check
        if (LowerValue.Contains(TEXT("charset=")) &&
            !LowerValue.Contains(TEXT("charset=utf-8"))) {
          return false;
        }
        if (LowerValue.Contains(TEXT("application/json")) ||
            LowerValue.Contains(TEXT("text/json"))) {
